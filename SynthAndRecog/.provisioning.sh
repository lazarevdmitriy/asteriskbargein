...
envsubst < ./SynthAndRecog/examples/mrcpconnectors.conf > /etc/asterisk/mrcpconnectors.conf
envsubst < ./SynthAndRecog/examples/tts.xml > /usr/local/unimrcp/conf/client-profiles/tts.xml
...
zcat ./SynthAndRecog/_phoneGetIVR.sql.gz | mysql -u $user --password=$pass -h $host $base
